...
import sys,json,os
from FlexUI.ViewerVideo.VideoThreadApp import VideoThread
from FlexUI.app_helper import spoint
import logging
import numpy as np
import cv2
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def swapSelect(self):
        ...
        myres={'midpoint':defaultdict(dict),'poly':defaultdict(dict),'box':defaultdict(dict)}
    ...
    def __init__(
        ...
        for dock in ["label_dock", "shape_dock"]:
    ...
    def setDirty(self):
        ...
        # title1='{} {}'.format(self.thread.curr_frame)
        # if self.filename is not None:
        title = "{} - {}*".format(title, self.title1)
    ...
    def saveLabels(self, filename=None):
        ...
        for instance in shapes:
            ...
            polygon_points=[[p.x(), p.y()] for p in instance.points]
            ...
            h,w=self.thread.h,self.thread.w
            c1=(center_x<w)
            c2=(center_y<h)
            if c1 & c2:
                view=0
            ...
            # print(center_x,h)
            # print(center_y,w)
            # print(view,instance.group_id)

            # Bounding box points
            bounding_box = np.array([min_x-w, min_y-h,max_x-w, max_y-h])
            midpoint=np.array([center_x-w,center_y-h])
            polygon_points1=np.array(polygon_points)-np.array([w,h])
            polygon_points=[polygon_points1.tolist()]
    ...
    def loadFile(self, filename=None):
        ...
        filename='/nfs/hpc/dgx2-6/tmp/2023/12/14a/labelme-main/FootBox/2.png'
    ...
    def loadFile1(self, image):
        frame=self.thread.curr_frame
        second = self.thread.curr_frame//self.thread.fps
        # self.title1="{}, {}:{}".format(frame, second//60, second % 60)
        a,b,c=second//3600, second//60, second % 60
        a,b,c=int(a),int(b),int(c)
        self.title1="{:02d}:{:02d}:{:02d} \t-\t {}".format(a,b,c, frame)
        ...
        if self._config["keep_prev"]:
            prev_shapes = self.canvas.shapes
        self.image = image
        self.canvas.loadPixmap(self.image)
        # flags = {k: False for k in self._config["flags"] or []}
        # print(self.thread.shapes)
        self.loadLabels(self.thread.shapes)
        # self.loadFlags(flags)
        if self._config["keep_prev"] and self.noShapes():
            self.loadShapes(prev_shapes, replace=False)
            self.setDirty()
